...
# flask imports not required for the project
...
# This class is used in the <<-- called from detect_objects() func
...
# draw boxes around the detected objects func <<-- called from detect_objects() func
...
# incoding the images after processing <<-- called from detect_objects() func
...
#flask app: Starting of the flask app when the domain is loaded
@app.route('/')
def upload():
  # calling sal's defined function
  sal_testFunc()
...
def post():
  ...
  if request.method == 'POST' and form.validate():
    # temporary photo file is created using the tempFile import
	with tempfile.NamedTemporaryFile() as temp:
      form.input_photo.data.save(temp)
      temp.flush()
	  # this is the call for object detection using tensorFlow
